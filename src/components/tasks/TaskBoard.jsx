...
const TaskBoard = ({ tasks, onEditTask, onDeleteTask, onAddTask }) => {
        ...
        <div key={status} className="flex flex-col space-y-4">
          <div className="flex items-center justify-between px-1">
            <div className="flex items-center space-x-2">
              <h3 className="font-semibold text-sm text-gray-700 uppercase tracking-wider uppercase">
                {getColumnTitle(status)}
              </h3>
              <Badge variant="secondary" className="h-5 px-1.5 text-[10px] font-bold bg-gray-100 text-gray-600">
          ...
          <div className="flex flex-col space-y-3 min-h-[500px] bg-slate-50/50 p-2 rounded-xl border border-slate-100/50">
            ...
            {columns[status].length === 0 && (
              <div className="flex-1 border-2 border-dashed border-slate-200 rounded-lg flex items-center justify-center p-8">
                <span className="text-xs text-slate-400 italic">No tasks here</span>
              </div>
            )}
