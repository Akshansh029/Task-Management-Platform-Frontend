  ...
  <Card className="overflow-hidden border-none shadow-sm bg-white hover:shadow-md transition-shadow">
    ...
    <CardContent className="p-6">
      <div className="flex items-center justify-between">
        ...
        <div className={`p-3 rounded-2xl ${colorClass.replace('bg-', 'bg-opacity-10 text-').replace('text-', 'text-opacity-100')}`}>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
        ...
        <Card className="border-none shadow-sm overflow-hidden rounded-2xl">
          <CardHeader className="flex flex-row items-center justify-between border-b bg-gray-50/50 px-6 py-4">
            ...
            <Link href="/projects" className="text-xs font-bold text-blue-600 hover:text-blue-700 flex items-center uppercase tracking-wider group">
              View all
                ...
                {recentProjects.map((project) => (
                  ...
                  >
                    ...
                    <div className="flex items-center text-gray-400 group-hover:text-blue-500">
                      <span className="text-[10px] mr-2 hidden sm:inline">Updated {formatDate(project.createdAt)}</span>
            ...
            {myTasks.length > 0 ? (
              <div className="divide-y divide-gray-100">
                {myTasks.map((task) => (
                  ...
                  >
                    <div className="flex items-center space-x-4">
                      <div className={`w-2 h-2 rounded-full ${task.priority === 'URGENT' ? 'bg-red-500 ring-4 ring-red-50' : 'bg-gray-300'}`} />
                      <div>
                        <h4 className="font-semibold text-gray-900 group-hover:text-blue-600 transition-colors line-clamp-1">{task.title}</h4>
                    ...
                    <div className="text-right">
                      <p className="text-xs font-bold text-gray-700">{task.project?.title}</p>
                      <p className="text-[10px] text-gray-500">{task.dueDate ? formatDate(task.dueDate) : 'No due date'}</p>
                    </div>
                  </Link>
                ))}
              </div>
              ...
              <div className="p-12 text-center">
                <p className="text-sm text-gray-500 italic">You have no tasks assigned to you.</p>
              </div>
            )}
          </CardContent>
        </Card>
